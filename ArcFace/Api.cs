...
namespace ArcFace
{
    ...
    public static class Api
    {
        ...
        /// <returns></returns>
        public static bool Init(out string message, string appId, string dKey, string rKey, int scale = 50, int maxFaceNumber = 100, string faceDataPath = "d:\\FeatureData")
        {
            if (scale < 2 || scale > 50)
            {
            ...
            if (maxFaceNumber < 1 || maxFaceNumber > 200)
            {
                message = "人脸数量必须在1-200之间";
                return false;
            }
            ...
            //ArcFace.ReadExcel.RegisterImg();
            var staffs = ArcFace.ReadExcel.ReadyData();
            int index = 0;
            var files = Directory.GetFiles(faceDataPath);
                    ...
                    foreach (var item in _FaceLib[taskIndex].Items.OrderByDescending(ii => ii.OrderId))
                    {
                        ...
                        foreach (var faceIndex in faceIndexs)
                        {
                            ...
                            if (score > 0.65)
                            {
                                ...
                                CacheFaceResults[faceIndex].Name = item.Name;
                                CacheFaceResults[faceIndex].TableNum = item.TableNum;
        ...
        public static void AddFace(string id, byte[] featureData, Image img, string name, string tablecode)
        {
            var fileName = Path.Combine(_FaceDataPath, id + ".dat");
            System.IO.File.WriteAllBytes(fileName, featureData);
            fileName = Path.Combine(_FaceImagePath, id + ".jpeg");
            img.Save(fileName);
            ...
            _FaceLib[0].Items.Add(new FaceLib.Item() { OrderId = DateTime.Now.Ticks, ID = id, FaceModel = faceModel, Name = name, TableNum = tablecode });
        }

        public static void InitFace(string id, byte[] featureData, Image img, string name, string tablecode)
        {
            var fileName = Path.Combine(_FaceDataPath, id + ".dat");
            System.IO.File.WriteAllBytes(fileName, featureData);
